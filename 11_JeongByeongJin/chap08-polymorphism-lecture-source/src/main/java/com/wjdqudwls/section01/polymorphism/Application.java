...
public class Application {
  ...
  public static void main(String[] args) {
    ...
    /* 다형성 : 하나의 인스턴스가 여러 가지 타입을 가질 수 있는 것
    *
    ...
    * */
    ...
    //[정적(프로그램 실행 전 == 컴파일 단) 바인딩]
    // - 매서드 호출부가 참조 변수의 타입에 맞춰서 연결됨
    // 프로그램 실행 전 animal1.move() -> 클래스의 move()

    // [동적 바인딩]
    // - 실제 인스턴스의 타입에 맞춰서 메서드가 연결
    // 프로그램 실행 중 animal1.move() -> Tiger에 오버라이딩한 move()

    animal2.move(); // 컴파일 단계 : Animal.
    animal3.move();
...
//    animal3.jump();
    ...
    *  == a가 참조하고 있는 인스턴스가 B타입을 상속하고 있는지 확인
    *  ==
    * - 맞으면 true, 아니면 false
    *
    * - 결과가 true인 경우
