package org.ho.section01.generic;
    ...
    /* 제네릭 프로그래밍
     - 데이터 형식의 의존 없이 여러 데이터 타입을 가질 수 있는 기술
      ...
      1. 컴파일 단계에서 강력한 타입 검사를 수행하여 타입 안정성 확보
         - 입력받은 Type(Integer)으로 설정한 제네릭 클래스에서 다른 Type(String)이 들어오면 바로 오류 발생시켜서 안정성 확보
...
//    gt2.setValue(100); //타입 오류
    ...
    *    자동으로 Object로 설정됨*/
    ...
    Integer i = (Integer)gt3.getValue(); //부모(Object)를 다운캐스팅(Integer)
    gt3.setValue("백");
    String s = (String)gt3.getValue();
    gt3.setValue(new int[]{1,2,3});
  }
}
