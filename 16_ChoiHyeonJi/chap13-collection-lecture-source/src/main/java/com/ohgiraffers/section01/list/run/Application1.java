...
public class Application1 {
  public static void main(String[] args) {
    ...
    * - List 인터페이스의 하위의 다양한 구현체로 변경이 가능하므로
    *   유연한 코드를 작성할 수 있다. */
    ArrayList alist = new ArrayList(); // 자식타입 = 자식 인스턴스
    List list = new ArrayList(); // 부모타입 = 자식 인스턴스 (업캐스팅) // ArrayList -> LinkedList 바꿔도 가능
    ...
    for(String str : slist){
      System.out.println(str.length()); // 무조건 String -> String 전용 메서드 호출 가능
    }
    ...
    olist.add("Hello");
    olist.add("world");
    olist.add(123); // int -> Integer로 변경되어 저장


    // 한 타입이 아니라 여러 타입 저장하는거라 검사하고 다운캐스팅 적용해야 함. -> 불편
    ...
    // 2) int size() : List에 저장된 요소의 개수를 반환 (길이 X)
    System.out.println("fruits.size() = " + fruits.size());

    // 3) E get(int index) : index번째 요소를 반환 //리스트에선 length x size o
    // for문 활용
    ...
    // 4) void add(int index, E e) : index 번쨰에 요소(e) 추가
    // - 해당 인덱스를 기준으로 뒤쪽 요소는 1칸씩 밀림  오른쪽으로 밀림
    fruits.add(1, "mango");
    System.out.println("fruits = " + fruits);

    // 5) E set(int index, E e) : index 번째의 요소를 e로 변경
    // 이 떄, 기존 요소가 반환됨. //요소 수정(변경)
    ...
    // Collections.reverse(fruits); //내림차순 정렬 뒤집기
    System.out.println("fruits = " + fruits);
